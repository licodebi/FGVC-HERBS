...
class Selector(nn.Module):
    ...
    def forward(self, x, logits=None):
        ...
        probs = torch.softmax(logits['layer3'], dim=-1)
        sum_probs = torch.softmax(logits['layer3'].mean(1), dim=-1)
        selections['layer3'] = []
        preds_1 = []
        preds_0 = []
        num_select = self.num_select['layer3']
        for bi in range(logits['layer3'].size(0)):
            _, max_ids = torch.max(sum_probs[bi], dim=-1)
            confs, ranks = torch.sort(probs[bi, :, max_ids], descending=True)
            sf = x['layer3'][bi][ranks[:num_select]]
            nf = x['layer3'][bi][ranks[num_select:]]  # calculate
            selections['layer3'].append(sf)  # [num_selected, C]
            preds_1.append(logits['layer3'][bi][ranks[:num_select]])
            preds_0.append(logits['layer3'][bi][ranks[num_select:]])
            if bi >= len(self.thresholds['layer3']):
                self.thresholds['layer3'].append(confs[num_select])  # for initialize
            else:
                self.thresholds['layer3'][bi] = confs[num_select]
        selections['layer3'] = torch.stack(selections['layer3'])
        preds_1 = torch.stack(preds_1)
        preds_0 = torch.stack(preds_0)
        logits["select_layer3"] = preds_1
        logits["drop_layer3"] = preds_0
        # 返回选择器
        return selections
...
class PluginMoodel(nn.Module):
    def __init__(self,
        ...
        total_num_selects=num_selects['layer3']+1
        # 设置图卷积聚合器
    ...
    def build_fpn_classifier(self, inputs: dict, fpn_size: int, num_classes: int):
        ...
        # 用于最后一层进行分类
        self.add_module("fpn_classifier_layer3", m)
        # x为输入的图片
    def fpn_predict_vit_down(self, x: dict, logits: dict):
        logit=x['layer3'][:,1:,:]
        logits['layer3'] = getattr(self, "fpn_classifier_layer3")(logit)
    ...
    def forward(self, x: torch.Tensor):
        ...
        # 取最后一层
        y['layer3']=outs['layer3'][:,1:,:]
        # 对最后一层进行分类
        self.fpn_predict_vit_down(outs, logits)
        ...
        selects['layer3'] = torch.cat([class_token[-1], selects['layer3']], dim=1)
